...
# FFmpeg setup (Auto-detect usually works well with pydub in Docker)
...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if file_obj.file_size > 49 * 1024 * 1024: # 50MB limit safe margin
    ...
    try:
        ...
        await show_main_menu(status_msg, is_video)
        
    except Exception as e:
        logger.error(f"Download Error: {e}")
        await status_msg.edit_text("❌ Download failed.")

async def show_main_menu(message, is_video=False):
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Sub Menus ---
        ...
        buttons = [
            [InlineKeyboardButton(v, callback_data=f"set_fmt_{k}") for k, v in list(AUDIO_FORMATS.items())[i:i+3]] 
            for i in range(0, len(AUDIO_FORMATS), 3)
        ]
        ...
        buttons = [
            [InlineKeyboardButton(v, callback_data=f"set_bit_{k}") for k, v in list(BITRATES.items())[i:i+3]]
            for i in range(0, len(BITRATES), 3)
        ]
        buttons.append([InlineKeyboardButton("🔙 Back", callback_data="back_main")])
...
async def process_audio_thread(query, context):
    """Runs processing in a separate thread to avoid blocking"""
    user_id = query.from_user.id
    session = user_sessions.get(user_id)
    
    await query.edit_message_text("⚙️ Processing... This may take a moment.")
    
    # Use asyncio.to_thread to run CPU-bound task
    try:
        ...
        if os.path.exists(session['input_file']): os.remove(session['input_file'])
        del user_sessions[user_id]
        
    except Exception as e:
        logger.error(f"Processing Error: {e}")
        await query.edit_message_text(f"❌ Error processing file: {str(e)}")
        if session and os.path.exists(session['input_file']):
             os.remove(session['input_file'])

def process_audio_logic(session):
    """Actual Pydub Logic (Blocking code)"""
    input_path = session['input_file']
    fmt = session['format']
    
    # Load Audio (handles video too automatically via ffmpeg)
    ...
    if session['speed'] != 1.0:
        # Complex speedup preserves pitch somewhat better than raw frame rate change
        audio = speedup(audio, playback_speed=session['speed'])

    # 3. Bass Boost (Simple Low Pass Filter Mix)
    if session['bass_boost']:
        # Create a bass track by filtering high freqs
        bass_line = audio.low_pass_filter(120)
        # Boost bass volume and overlay
        bass_line = bass_line.apply_gain(6)
    ...
    if session['compress']:
        bitrate = "64k" # Force low for compression
    ...
    caption = (
        f"✅ *Completed*\n"
        f"Format: {fmt.upper()} | Bitrate: {bitrate}\n"
        f"Speed: {session['speed']}x | Bass: {'On' if session['bass_boost'] else 'Off'}"
    )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_sessions and user_sessions[user_id].get('waiting_for_trim'):
        try:
            ...
            await update.message.reply_text(f"✅ Trim Set: {start}s to {end if end > 0 else 'End'}s")
            await show_main_menu(update.message)
        except:
            await update.message.reply_text("❌ Invalid format. Use `0 0` to cancel or `10 20` for time.")
