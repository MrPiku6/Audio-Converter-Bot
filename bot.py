...
# from . import app # Import app from the current module for gunicorn (Not needed for this structure)
...
def home():
	...
	# Flask server is now run by Gunicorn, so these functions are no longer needed.
	# def run_flask():
...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        user_sessions[user_id] = {
            ...
            'fade': False,
            'gain': 0.0, # New feature: Volume/Gain in dB
            'reverb': False, # New feature: Reverb effect
            'is_video': is_video,
...
async def show_main_menu(message):
    ...
    text = (
        ...
        f"• Volume: `{session.get('gain', 0.0):+.1f} dB`\n"
        f"• Reverb: `{'✅' if session.get('reverb') else '❌'}`\n
    )
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔊 Volume", callback_data="menu_volume"),
         InlineKeyboardButton("🌌 Reverb", callback_data="toggle_reverb")]
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "toggle_normalize":
        ...
        await show_main_menu(query.message)
	        ...
	        await show_main_menu(query.message)
	    elif data == "toggle_reverb":
	        session['reverb'] = not session['reverb']
	        await show_main_menu(query.message)
        ...
        await query.edit_message_text("Select Playback Speed:", reply_markup=InlineKeyboardMarkup(buttons))
	        ...
	        gains = [-10, -5, 0, 5, 10]
	        buttons = [[InlineKeyboardButton(f"{g:+.1f} dB", callback_data=f"set_gain_{g}")] for g in gains]
	        buttons.append([InlineKeyboardButton("🔙 Back", callback_data="back_main")])
	        await query.edit_message_text("Select Volume Change (Gain in dB):", reply_markup=InlineKeyboardMarkup(buttons))
        ...
        await show_main_menu(query.message)
	        ...
	        await show_main_menu(query.message)
	    elif data.startswith("set_gain_"):
	        session['gain'] = float(data.split("_")[2])
	        await show_main_menu(query.message)
    elif data == "back_main":
...
def process_audio_logic(session):
    ...
    # Handle video input by extracting audio stream
    if session.get('is_video'):
        # pydub can handle video files by default, but we'll ensure it's treated as audio
        # by using the from_file method which relies on ffmpeg to extract the audio stream.
        # No explicit extraction step is needed if pydub is configured correctly.
        # We'll just load it.
        pass
    ...
    if session['bass_boost']:
	    ...
	    if session['gain'] != 0.0:
	        audio = audio + session['gain']
	
	    if session['reverb']:
	        # Simple echo/delay effect as a substitute for reverb
	        # Create a delayed, attenuated version of the audio
	        delay = 200 # milliseconds
	        attenuation = 6 # dB
	        delayed_audio = audio - attenuation
    ...
    output_filename = f"processed_{os.path.splitext(session['original_name'])[0]}.{fmt}"
	    ...
	    caption = f"✅ *Completed*\nFormat: {fmt.upper()} | Bitrate: {bitrate}\nSpeed: {session['speed']}x | Volume: {session['gain']:+.1f} dB | Reverb: {'✅' if session['reverb'] else '❌'}"
    ...
    if user_id in user_sessions and user_sessions[user_id].get('waiting_for_trim'):
        try:
            ...
            if start < 0 or end < 0 or (end > 0 and start >= end):
                await update.message.reply_text("❌ Invalid time range. Start time must be less than end time, and both must be non-negative.")
                return

            user_sessions[user_id]['trim_start'] = start
            user_sessions[user_id]['trim_end'] = end if end > 0 else None
            user_sessions[user_id]['waiting_for_trim'] = False
            await update.message.reply_text(f"✅ Trim Set: {start}s to {user_sessions[user_id]['trim_end'] or 'End'}!")
            await show_main_menu(update.message)
        except:
...
def main():
    if not BOT_TOKEN:
        ...
        return
	    # start_keep_alive() # Gunicorn will handle the web server
